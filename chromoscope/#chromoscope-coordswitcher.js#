	...
	function init(chromo) {
		...
		chromo.switchCoordinateSystem = function(coord){
			...
			for(var s = 0; s < this.spectrum.length; s++){
			    ...
			    console.log(this.spectrum[s].tiles)
			}
			for(var a = 0; a < this.annotations.length; a++){
				if(coord == "A"){
					if(!this.annotations[a].tiles_gal) this.annotations[a].tiles_gal = this.annotations[a].tiles;
					if(this.annotations[a].tiles_eq) this.annotations[a].tiles = this.annotations[a].tiles_eq;
				}else{
					if(this.annotations[a].tiles_gal) this.annotations[a].tiles = this.annotations[a].tiles_gal;
				}				
			    console.log(this.annotations[a].tiles)
			}
			this.updateCoords();
			this.updateImages();
		}
		...
		chromo.updateImages = function(){
			...
			if(this.wavelength_load_range > 0){
				for(var w = 1; w <= this.spectrum.length ; w++){
					// Check if the lower wavelength is required
				    if(layers[0]-w >= 0) layers[l++] = layers[0]-w;
				    // Check if the higher wavelength is required
				    if(layers[0]+w <= this.spectrum.length-1) layers[l++] = layers[0]+w;
				}
			}
		    ...
		    console.log('loading ',layers.length,' layers')
			for(var l = 0 ; l < layers.length ; l++){
				...
				for (var v = 0; v < visibleTiles.length; v++, counter++) {
					...
					else tileName = this.id+"_"+this.spectrum[-(idx+1)].name+"x" + visibleTiles[v].x + "y" + visibleTiles[v].y + "z"+this.zoom;
		    ...
		    for(var a = 0 ; a < this.annotations.length ; a++){
			output = "";
			//var idx = annotations[a];
			
			// Loop over all the tiles that we want to load
			for (var v = 0; v < visibleTiles.length; v++, counter++) {
			    if(a >= 0) tileName = this.id+"_"+this.annotations[a].name+"x" + visibleTiles[v].x + "y" + visibleTiles[v].y + "z"+this.zoom;
			    else tileName = this.id+"_"+this.annotations[-(a+1)].name+"x" + visibleTiles[v].x + "y" + visibleTiles[v].y + "z"+this.zoom;
			    
			    this.visibleTilesMap[counter] = tileName;
			    
			    var tiles = this.annotations[a].tiles;
			    tiles = (typeof tiles=="string") ? tiles : (typeof tiles["z"+this.zoom]=="string") ? tiles["z"+this.zoom] : tiles.z;
			    var img = (inrange) ? this.cdn+tiles+visibleTiles[v].src+'.'+this.annotations[a].ext : this.annotations[a].blank;
