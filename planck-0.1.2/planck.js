 ...
 * 
 * This application requires Chromoscope. It will run locally or on a web server. To run locally you will need to download the appropriate tile sets and code.
 * 
 * Functions in v0.1.2
 *   - No longer uses registerKey plugin as functionality moved into Chromoscope
 *
 * Functions in v0.1.1
 ...
 *   - Uses registerKey plugin to allow behaviour of keys to be changed
...
function setupPlanck(){    
    ...
    chromo.registerKey('.',function(){
    	console.log('press .')
		//reregister hide keys to include kiosk and overlay
		$(this.body+" h1").toggle();
		$(this.body+" h2").toggle();
    ...
    chromo.registerKey(45,function(){ // user presses the - (45 for Firefox)
		//decrease magnification
		this.changeMagnification(-1);
    });
    chromo.registerKey('c',function(){
	...
	// Re-build the help screen now that we've added new keys
	chromo.buildHelp(true);	
